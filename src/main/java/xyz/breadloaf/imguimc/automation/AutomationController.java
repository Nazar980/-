...
public class AutomationController {
    ...
    private void craftEmeraldPickaxe(Minecraft client) {
        ...
        if (result.isEmpty() || result.getItem() != Items.EMERALD_PICKAXE) {
        ...
        setStage(Stage.EQUIP_PICKAXE, "Emerald pickaxe crafted", 8);
    }
    ...
    private void equipEmeraldPickaxe(Minecraft client) {
        if (client.player.getMainHandItem().getItem() == Items.EMERALD_PICKAXE) {
            setStage(Stage.SELL_PICKAXE, "Emerald pickaxe equipped", 2);
            return;
        }

        AbstractContainerMenu menu = client.player.containerMenu;
        int pickaxeSlot = findInventorySlot(menu, Items.EMERALD_PICKAXE);
        if (pickaxeSlot == -1) {
            setStage(Stage.EVALUATE, "No emerald pickaxe to equip", 4);
            return;
        }
        ...
        setStage(Stage.SELL_PICKAXE, "Equipping emerald pickaxe", 6);
    }

    private void sellEmeraldPickaxe(Minecraft client) {
        if (client.player.getMainHandItem().getItem() != Items.EMERALD_PICKAXE) {
            setStage(Stage.EQUIP_PICKAXE, "Pickaxe is not in hand", 4);
            return;
        }
    ...
    private boolean hasEmeraldPickaxe(Minecraft client) {
        ...
        for (ItemStack stack : client.player.getInventory().items) {
            if (!stack.isEmpty() && stack.getItem() == Items.EMERALD_PICKAXE) {
                return true;
            }
        }
        return false;
